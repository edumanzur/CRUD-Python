...
def criar_estoque():
    try:
        ...
        if not nome or not autor or not quantidade_str:
            print("Erro: Todos os campos são obrigatórios!")
            return
        ...
        print(f"Estoque para '{nome}' criado com sucesso!")
        
    except con.DatabaseError as erro:
        print(f"Erro ao criar estoque: {erro}")
    finally:
        fechar_conexao(conexao)
...
def listar_estoque():
    ...
    try:
        ...
        cursor.execute("SELECT ID_Estoque, Nome, Autor, Quantidade FROM Estoque")
        ...
        print("\n--- Lista de Estoque ---")
        print(f"{'ID':<4} {'Nome':<25} {'Autor':<25} {'Quantidade':<10}")
        print("-" * 70)
        
        for estoque in estoques:
            id_estoque, nome, autor, quantidade = estoque
            print(f"{id_estoque:<4} {nome:<25} {autor:<25} {quantidade:<10}")
            
    except con.DatabaseError as erro:
        print(f"Erro ao listar estoque: {erro}")
    finally:
        fechar_conexao(conexao)
